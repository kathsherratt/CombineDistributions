...
```{r outlier-plot-crps-peak, fig.width=8, fig.height = 3, echo = FALSE}
...
crps_peak_best_outlier <- obs %>%
  ...
  theme(legend.position = "bottom", 
        ...
        strip.background = element_blank(), 
        strip.placement = "outside")
crps_peak_best_outlier
```

```{r Fig3, include = FALSE, eval = FALSE, fig.width=12, fig.height = 10}
...
```{r Fig4, include = FALSE, eval = FALSE, fig.width=12, fig.height = 4}
F4a = pdfs %>% 
  filter(variable == "peak_cases") %>%
  mutate(struc_uncert = factor(struc_uncert, levels = names(labs_struc_super_simp))) %>%
  ggplot(aes(x = x, y = y)) +
...
```{r Fig5, include = FALSE, eval = FALSE}
...
```{r supp-cum, include = FALSE, eval = FALSE}
...
```{r supp-peak, include = FALSE, eval = FALSE}
# save into supplemental figures
...
```{r supp-othercase, include = FALSE, eval = FALSE}
...
```{r values-for-text, include = FALSE, eval = FALSE}
...
```{r outlier-values-for-text, include = FALSE, eval = FALSE}
